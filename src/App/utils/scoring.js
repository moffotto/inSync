export function checkPointsHeaders(response) {
    let headers = response.headers;

    if (headers.has('Challengename')) {
        createCustomAlert(headers.get("Challengedescription"), headers.get("Challengepoints"));
    }
}

function createCustomAlert(description, points) {
	let d = document;
	...
	h1.appendChild(d.createTextNode("Hacked for " + points + " points!"));
